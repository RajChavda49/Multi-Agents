...
function findRelevantFiles(task, tree) {
  const tokens = keywordTokens(`${task.summary} ${task.description}`);
...
export function gatherKnowledgeContext(jiraTask) {
  ...
  const matched = findRelevantFiles(jiraTask, tree);
  ...
  return {
    ...
    codebase_notes: `Indexed ${tree.length} paths from ${repoPath}. ${matched.length} files matched Jira task keywords.`,
...
export function writeFilesToTargetRepo(files) {
  ...
  return { root, written, skipped: false };
}
